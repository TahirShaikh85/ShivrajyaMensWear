...
export default function DeliveryTimeline() {
  ...
  if(trackedOrderFailed){
    return <div className="mt-8 text-red-400">{trackedOrderFailed.error}</div>
  }
  ...
  function generateTimelineItems () {
    ...
    for (let i = 0; i <= currentStatusIndex; i++) {
      ...
      timelines.push(renderTimelineItem(text, level, statusIcon, color));
    }
    ...
    <>
      <Fade delay={2000} triggerOnce={true}>
        <div className="track-orderDetail flex mt-10 py-10 px-5 border rounded shadow-xl">
          ...
          <div><img className="w-32" src="https://i.dummyjson.com/data/products/2/thumbnail.jpg" alt="" /></div>

          <div className="flex flex-col ml-6 mr-4 text-sm md:text-lg">
            <span className="title">Lenovo idead pad  </span>
            <span className="brand">Lenovo</span>
            <div className="flex mt-2 text-sm" >
              <p className=""><span className="bold">Size</span>:  M</p>
              <p className="ml-4"><span className="bold">Quantity</span>: 2</p>
            </div>
            <p className="text-sm mt-2 "><span className="text-blue-400">Amount: </span> ₹900</p>
            <p className="text-sm mt-1"><span className="text-orange-400">Ordered On: </span> 23-12-22</p>
          </div>
